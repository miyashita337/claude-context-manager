...
### Step 6: `/create-pr` ワークフローへ自動移行

調査レポートを出力したら、**ユーザーへの確認なしに自動的に** create-pr ワークフローを開始する。

> **理由**: `/investigate` はユーザーが手動で呼ぶ起点コマンド。
> 調査完了後に実装へ進むのが自然なフローであり、二度手間を排除する。

以下のメッセージをユーザーに表示してから create-pr ワークフローを実行する:
