...
```text
[検知層]                    [記録層]                           [レポート層]
JSONL (tail)           →    ~/.claude/guardrails/              →    自作 exporter
                            violations.jsonl  (repo外・既存)
  ↓                                                                  ↓
...
`violations.jsonl` は**プロジェクトルート配下に置かない**。理由：
- セッション内容には個人情報・機密情報が含まれる可能性
- プロジェクト内に置くと `.gitignore` 漏れで git commit 事故のリスク（RW-005 類似）
- 複数プロジェクト横断で参照するため、プロジェクト依存にしない方が自然

**実装上の配置**: `~/.claude/guardrails/violations.jsonl`（既存 `src/hooks/guardrail_log.py` が使用済みパスに合流）
- 既存のローテーション仕様（>10MB で `violations-YYYYMMDD-HHMMSS.jsonl` に rotate）を踏襲
- 本 Epic で追加する attention hijack 検知イベントも同ファイルに `type: attention_hijack` として追記する前提
