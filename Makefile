...
# Skills インストール（global: true マーカー付きのみ）
SKILLS_SRC = .claude/skills
SKILLS_DEST = $(HOME)/.claude/skills

install-skills: ## global: true マーカー付きSkillsを ~/.claude/skills/ にコピー（個別承認）
	@echo "=== Skills Install ==="
	@echo ""
	@UNREV=0; \
	for skill_dir in $(SKILLS_SRC)/*/; do \
		has_global=$$(grep -c "^global:" "$$skill_dir/SKILL.md" 2>/dev/null || echo 0); \
		if [ "$$has_global" = "0" ]; then UNREV=$$((UNREV+1)); fi; \
	done; \
	if [ "$$UNREV" -gt 0 ]; then \
		echo "⚠  $$UNREV skill(s) に global マーカーなし。'make skill-gate' で判定してください。"; \
		echo ""; \
	fi
	@CHANGED=0; \
	for skill_dir in $(SKILLS_SRC)/*/; do \
		skill=$$(basename $$skill_dir); \
		if grep -q "^global: true" "$$skill_dir/SKILL.md" 2>/dev/null; then \
			dest="$(SKILLS_DEST)/$$skill"; \
			if [ ! -d "$$dest" ]; then \
				echo "[NEW]     $$skill"; \
				CHANGED=1; \
			elif ! diff -q "$$skill_dir/SKILL.md" "$$dest/SKILL.md" >/dev/null 2>&1; then \
				echo "[CHANGED] $$skill"; \
				diff "$$dest/SKILL.md" "$$skill_dir/SKILL.md" 2>/dev/null | head -20; \
				echo ""; \
				CHANGED=1; \
			else \
			...
			echo "[SKIP]    $$skill (project-only)"; \
		fi; \
	done; \
	echo ""; \
	if [ "$$CHANGED" = "0" ]; then \
		echo "All skills up to date. Nothing to install."; \
	else \
		for skill_dir in $(SKILLS_SRC)/*/; do \
			skill=$$(basename $$skill_dir); \
			if grep -q "^global: true" "$$skill_dir/SKILL.md" 2>/dev/null; then \
				dest="$(SKILLS_DEST)/$$skill"; \
				if [ ! -d "$$dest" ] || ! diff -q "$$skill_dir/SKILL.md" "$$dest/SKILL.md" >/dev/null 2>&1; then \
					read -p "Install skill '$$skill'? (y/N): " confirm; \
					if [ "$$confirm" = "y" ] || [ "$$confirm" = "Y" ]; then \
						mkdir -p "$$dest"; \
						cp "$$skill_dir"/* "$$dest/" 2>/dev/null; \
						if [ -d "$$skill_dir/references" ]; then \
							mkdir -p "$$dest/references"; \
							cp "$$skill_dir/references/"* "$$dest/references/" 2>/dev/null; \
						fi; \
...
skill-gate: ## 新規Skillのグローバル適性を自動判定
	@echo "=== Skill Global Gate Check ==="
	@echo ""
	@for skill_dir in $(SKILLS_SRC)/*/; do \
		skill=$$(basename $$skill_dir); \
		has_global=$$(grep -c "^global:" "$$skill_dir/SKILL.md" 2>/dev/null || echo 0); \
		if [ "$$has_global" = "0" ]; then \
			echo "[UNREVIEWED] $$skill — global マーカーなし"; \
			REFS=$$(grep -cE "PITFALLS|make pre-git|make startup|context-history|topic-server" "$$skill_dir/SKILL.md" 2>/dev/null || echo 0); \
			OVERLAP=$$(ls $(SKILLS_DEST)/$$skill/SKILL.md 2>/dev/null && echo "1" || echo "0"); \
			if [ "$$REFS" -gt 0 ]; then \
				echo "  → プロジェクト固有参照あり ($$REFS箇所) → global: false 推奨"; \
			elif [ "$$OVERLAP" = "1" ]; then \
				echo "  → ~/.claude/skills/$$skill に既存Skill重複 → global: false 推奨"; \
			else \
				echo "  → 汎用的な内容 → global: true 候補"; \
			fi; \
			echo "  アクション: SKILL.md に 'global: true' または 'global: false' を追加してください"; \
			echo ""; \
		fi; \
	done; \
	UNREVIEWED=$$(for d in $(SKILLS_SRC)/*/; do grep -cL "^global:" "$$d/SKILL.md" 2>/dev/null; done | grep -c "0" || true); \
	echo "未レビュー: $$UNREVIEWED skills"

install-all: install install-skills ## Token Analyzer + Skills を一括インストール
